...
namespace SeleniumCsharp
{
    public class Tests
    {
        ...
        IWebDriver driver;
        String test_url = "https://pavelivanov22.thkit.ee/Database/KaupadeDatabase/kaubahaldus.php";
        ...
        public void test_page1()
        {
           // driver.Url = test_url;
            ...
            try { IWebElement sButton2 = driver.FindElement(By.XPath("//button[@class='']")); sButton2.Click(); } catch (Exception) {}

            for (int a = 0; a < 1; a++)
            {
                Thread.Sleep(2500);
                var sRadio = driver.FindElements(By.XPath("//div[@class='']"));
                for (int i = 0; i < sRadio.Count; i++)
                {
                    var els = sRadio[i].FindElements(By.XPath(".//input[@type='submit']"));
                    if (els.Count >= 2)
                    {
